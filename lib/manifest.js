...
var serverManifest = function (auth, guid, cf) {
  return cf.apps.getSummary(auth.token_type, auth.access_token, guid).then(function (summary) {
    var manifest = {},
        application = {};
...
buildManifest = function (cfBlob) {
  return localManifest().then(function (localM) {
    if (cfBlob.app.guid) {
      return serverManifest(cfBlob.auth, cfBlob.app.guid, cfBlob.cf).then(function (serverM) {
